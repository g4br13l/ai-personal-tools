...
**AI Personal Tools** is a collection of AI-powered tools designed to automate repetitive tasks and boost productivity.

This repository implements that vision as a **Bun workspaces** monorepo: shared packages for domain contracts (`@repo/core`), scraping and crawling (`@repo/agent`), persistence (`@repo/db-ai`), cross-cutting utilities (`@repo/infra`), and TypeScript presets (`@repo/tsconfig`). The main interactive surface today is **`ai-personal-tools-cli`**, an Ink + React terminal app composed on top of those packages.
...
### End-to-end flow (job extraction)
...
| Location | Notes |
| --- | --- |
| `ai-personal-tools-cli` | `bun test` — e.g. `src/modules/jobPostingExtract/.../jobPostingExtract.schema.test.ts` (Zod/metadata alignment with `@repo/core`). No dedicated `test` script in `package.json`; invoke `bun test` directly. |
